...
An interactive scatterplot that shows the pollution (concentration of PM2.5),
...
function draw(){
    ...
    d3.json("data.json", function(error, data) {
    	...
    	y.domain(d3.extent(data, function(d) { return d.pollution; })).nice();
    	...
    	// Apply dots
    	plot.selectAll(".dot")
    		.data(data.filter(function(d){ if(d.pollution && d.lifeExpectancy) return d ; }))
            .enter().append("circle")
        		.attr("class", "dot")
        		.attr("r", scaleDots)
        		.attr("cx", function(d) { return x(d.lifeExpectancy); })
        		.attr("cy", function(d) { return y(d.pollution); })
        ...
        function handleMouseOver(d, i) {
            ...
            //Specify where to put label of text and apply text for pollution
            plot.append("text")
                .attr("id", "info")
                .attr("x", 60)
                .attr("y", 80)
                ...
                .text( function() {
    			    return "Pollution: " + d.pollution + " pm2.5";
    		});
